...
#Last Updated: 2/13/2021 Nicole Harty
...
## Perform common factor manipulations (reordering, collapsing factors) and work with strings (trucating, matching)
...
SurveyXLSX %>%
  select(Timestamp) %>%
  filter(Timestamp < "2021-03-15")

#alternative, base R, no PIPE: read inside to outside parenthesis - UCK!
filter(select(SurveyXLSX, Timestamp), Timestamp < "2021-03-15")
...
fct_recode(SurveyXLSX$ExperienceProgCode, "minimal" = "Novice")
